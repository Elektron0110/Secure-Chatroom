...
async function uploadChatAvatar(chatId, file) {
    ...
    formData.append('avatar', file);
    try {
        const data = await apiForm('POST', `/api/chats/${chatId}/avatar`, formData);
        // Обновляем аватар чата в state
        const chatIndex = state.chats.findIndex(c => c.id === chatId);
        if (chatIndex !== -1) {
            state.chats[chatIndex].avatarUrl = data.avatarUrl;
        }
        renderChatList();
        ...
        showToast(e.message || 'Ошибка загрузки аватара группы');
    }
}
...
function updateAvatarDisplays() {
    ...
    if (sidebarAvatar) {
        if (url) {
            ...
            sidebarAvatar.innerHTML = `<div class="initials">${(state.user.displayName || state.user.username || '?').charAt(0).toUpperCase()}</div>`;
        }
    }
    updateProfileAvatar();
}
        ...
        try {
            ...
            if (msg.type === 'message') {
                if (msg.chatId === state.currentChatId) {
                    ...
                    // Новое сообщение в другом чате — показываем уведомление с аватаром
                    const chat = state.chats.find(c => c.id === msg.chatId);
                    if (chat) {
                        const chatName = getChatName(chat);
                        const chatAvatar = chat.avatarUrl || (chat.isGroup ? null : getOtherParticipant(chat)?.avatarUrl);
                        showNotification(`Новое сообщение в чате "${chatName}"`, chatAvatar);
                    }
                }
                loadChats();
...
function showNotification(message, iconUrl = null) {
    const icon = iconUrl || '/static/fav.jpg';
...
async function loadMessages(chatId) {
    try {
        ...
        if (JSON.stringify(state.messages) !== JSON.stringify(msgs) && flag && document.hidden) {
            ...
            const chatAvatar = chat ? (chat.avatarUrl || (chat.isGroup ? null : getOtherParticipant(chat)?.avatarUrl)) : null;
            showNotification(`У Вас новое сообщение в чате "${chatName}"`, chatAvatar);
...
function renderChatList() {
    ...
    if (!chats.length) {
        list.innerHTML = `<div style="padding:40px;text-align:center;color:var(--text-secondary);">${q ? 'Ничего не найдено' : 'Нет чатов. Нажмите + чтобы начать'}</div>`;
        return;
    }
    ...
    list.innerHTML = chats.map(chat => {
        ...
        const badge = chat.unreadCount > 0 ? `${chat.unreadCount > 99 ? '99+' : chat.unreadCount}` : '';
        ...
        // Определяем аватар для отображения в списке чатов
        let avatarContent;
        if (chat.isGroup) {
            // Для групп используем avatarUrl чата
            avatarContent = chat.avatarUrl
                ? `<img src="${chat.avatarUrl}" alt="Avatar">`
                : initials;
        } else {
            // Для личных чатов используем аватар собеседника
            ...
            <div class="chat-item${active}" data-id="${chat.id}">
                ...
                <div class="chat-info">
                    ...
                    <div class="chat-info-bottom">
                        ...
                        ${badge ? `<span class="unread-badge">${badge}</span>` : ''}
                    </div>
                </div>
            </div>
        `;
...
function renderMessages() {
    ...
    if (!state.messages.length) {
        container.innerHTML = '<div style="flex:1;display:flex;align-items:center;justify-content:center;color:var(--text-secondary);">Нет сообщений</div>';
        return;
    }
    ...
    container.innerHTML = state.messages.map(msg => {
        ...
        const dateSep = dateStr !== lastDate ? `<div class="date-separator">${dateStr}</div>` : '';
        lastDate = dateStr;
        const senderName = (isGroup && !isSent && msg.sender)
            ? `<div class="msg-sender">${escHtml(msg.sender.displayName)}</div>` : '';
...
function appendMessage(msg, isSent) {
    ...
    const deleteBtn = isSent
        ? `<button class="msg-delete-btn" data-msg-id="${msg.id}" title="Удалить">×</button>` : '';
    div.innerHTML = `${deleteBtn}${senderName}
        <div class="msg-content">
            <div class="msg-text">${escHtml(msg.content)}</div>
            <div class="msg-meta">
                <span class="msg-time">${formatTimeShort(msg.createdAt)}</span>
...
function renderUsersList(users) {
    ...
    if (!users.length) {
        list.innerHTML = '<div style="padding:20px;text-align:center;color:var(--text-secondary);">Пользователи не найдены</div>';
        return;
    }
    list.innerHTML = users.map(u => {
        ...
        return `
            <div class="user-item${sel}${gm}" data-uid="${u.id}" data-name="${escHtml(u.displayName || u.username)}">
